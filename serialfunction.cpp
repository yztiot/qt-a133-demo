...
void SerialFunction::setSerialModel(int nModel){
    m_serialModel = nModel;
}
...
//        }
//触摸屏启用下面代码 A133的232接口为ttyUSB1
        if(m_serialModel == 0 && "ttyS2" == list.at(i).portName()){
            m_portName = list.at(i).portName();
        }else if(m_serialModel == 1 && "ttyUSB1" == list.at(i).portName()){

            m_portName = list.at(i).portName();

        }
        /*else if(m_serialModel == 1 && "ttyS1" == list.at(i).portName()){
            m_portName = list.at(i).portName();
        }*/
    }
}
...
/*
void SerialFunction::runOpenSerial(){
    QSerialPort  *ptPort = new QSerialPort;
    m_canSerialThread = new SerialThread(ptPort);

    m_canThread = new QThread;
    m_canSerialThread->moveToThread(m_canThread);

   // connect(m_canSerialThread, SIGNAL(sendSerialData(QString)), this, SLOT(readData(QString)));
    connect(this,SIGNAL(startCanSig()),m_canSerialThread,SLOT(starCanThread()));
    m_canThread->start();
    emit startCanSig();
}
*/
